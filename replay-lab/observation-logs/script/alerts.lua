...
local function is_not_fluid(name)
    local items = { "crude-oil", "petroleum-gas", "sulfuric-acid", "water", "steam", "light-oil", "heavy-oil",
        "lubricant" }
    ...
    if entity.type == "assembling-machine" then
        ...
        if recipe then
            ...
            if #missing_resources > 0 then
                ...
                recipe.name .. " due to missing resources: " .. table.concat(missing_resources, ",_") .. "\'"
            end
        end
    end
    return false
    ...
    if not has_output_space(entity) then
        if entity.drop_position then
            ...
            if destination_entity then
                if destination_entity.type == "container" then
                    table.insert(issues,
                        "\'chest at drop position is full. Empty the chest at (" ..
                        rounded_x .. ", " .. rounded_y .. ") to continue mining.\'")
                elseif destination_entity.type == "furnace" then
                    table.insert(issues, "\'furnace belt at drop position is blocked.\'")
                else
                    table.insert(issues,
                        "\'transport belt at drop position is blocked. Clear the belt at (" ..
                        rounded_x .. ", " .. rounded_y .. ") to continue mining.\'")
                end
            else
                table.insert(issues,
                    "\'output blocked by item on the ground. There is no sink entity in place to accept the output.\'")
            end
        else
            table.insert(issues, "\'waiting for space in destination\'")
        end
